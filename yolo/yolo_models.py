...
class WeightedLayer(object):
    def __init__(self: WeightedLayer):
        self.conv = None
        self.norm = None
        self.input_shape = None
        return

    def __str__(self: WeightedLayer):
        if self.conv is None:
            conv = 'none'
        else:
            conv = self.conv.__class__.__name__
        if self.norm is None:
            norm = 'none'
        else:
            norm = self.norm.__class__.__name__
        if self.input_shape is None:
            input_shape = 'not build'
        else:
            input_shape = str(self.input_shape)
        return f'conv:{conv}, norm:{norm}, input_shape:{input_shape}'
...
class YoloV3_tiny(tf.keras.Model):
    def __init__(self: YoloV3_tiny, nc: int) -> None:
        super().__init__()
        self.weighted_layers = list()
    ...
    def call(self: YoloV3_tiny, images: tf.Tensor) -> List[tf.Tensor]:
        x_8, x = self.net(images)
        x = self.dc(x)
        pred = x
...
class YoloV3(tf.keras.Model):
    def __init__(self: YoloV3, nc: int) -> None:
        super().__init__()
        self.weighted_layers = list()
        self.net = Darknet53()
        self.weighted_layers.extend(self.net.weighted_layers)
    ...
    def call(self: YoloV3, images: tf.Tensor) -> List[tf.Tensor]:
        x_36, x_61, x = self.net(images)
        x = self.dc1(x)
        pred = x
        large_bbox = self.yl1(pred)
...
class YoloV4(tf.keras.Model):
    def __init__(self: YoloV4, nc: int) -> None:
        super().__init__()
        self.weighted_layers = list()
    ...
    def call(self: YoloV4, images: tf.Tensor) -> List[tf.Tensor]:
        x_54, x_85, x = self.net(images)
        short_cut = x
